...
class ArtificialAnalysisProvider(BenchmarkProvider):
    ...
    def __init__(self):
        ...
        self.cache_ttl = int(os.getenv("ROUTER_ARTIFICIAL_ANALYSIS_CACHE_TTL", "86400"))
        self.mapping_file = os.getenv("ROUTER_ARTIFICIAL_ANALYSIS_MODEL_MAPPING_FILE")
        ...
        self._day_reset = datetime.now(UTC).date()
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0, verify=settings.verify_tls) as client:
                ...
                if data.get("status") != 200:
                    logger.error(f"ArtificialAnalysis API returned status: {data.get('status')}")
                    return []
                ...
                if models and not hasattr(self, "_debug_logged"):
                    ...
                    for m in models:
                        ...
                        if any(local in name for local in local_names):
                            matches.append(
                                (m.get("id"), m.get("name"), m.get("model_creator", {}).get("slug"))
                            )
                    if matches:
                ...
                for model_data in models:
                    ...
                    benchmark = self._convert_to_benchmark_dict(model_data, ollama_name)
                    if benchmark:
                ...
                logger.info(f"Converted {len(results)} ArtificialAnalysis models to benchmark data")
                return results
    ...
    def _is_cache_valid(self) -> bool:
        ...
        if not self._cache_timestamp:
            return False
        return (datetime.utcnow() - self._cache_timestamp) < timedelta(seconds=self.cache_ttl)
    ...
    def _map_to_ollama_name(
        ...
        aa_id = str(model_data.get("id", ""))  # May be missing in test fixtures
        aa_name = str(model_data.get("name", ""))
        creator = str(model_data.get("model_creator", {}).get("name", ""))
        ...
        if creator:
            ...
            model_slug = aa_name.lower().replace(" ", "-").replace(".", "").replace("_", "-")
    ...
    def _convert_to_benchmark_dict(
        ...
        try:
            ...
            if count > 0:
                ...
                coding_score = round(
                    coding / max(1, (1 if livecodebench is not None else 0) + 1), 3
                )
