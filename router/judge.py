...
class JudgeClient:
    ...
    async def score_response(self, prompt: str, response: str) -> float:
                ...
                if isinstance(e, httpx.HTTPStatusError):
                    ...
                    if status_code == 400:
                        try:
                            ...
                            logger.warning(
                                f"Judge 400 error (attempt {attempt + 1}/{self.max_retries}): {error_body[:500]}"
                            )
                        except Exception:
                            logger.warning(
                                f"Judge 400 error (attempt {attempt + 1}/{self.max_retries}): {e}"
                            )
                        ...
                        logger.warning(
                            f"Judge client error {status_code} on attempt {attempt + 1}/{self.max_retries}: {e}"
                        )
                        break
                else:
        ...
        if last_exception:
            ...
            logger.warning(
                f"Judge scoring failed after {self.max_retries} attempts: {error_msg}. Falling back to basic check."
            )
