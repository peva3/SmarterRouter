...
def mock_backend():
    ...
    async def mock_list_models():
        return [
            ModelInfo(
                name="llama3.2:1b", size=1321098329, modified_at="2026-02-15T16:35:08.8417444Z"
            ),
            ModelInfo(
                name="qwen2.5-coder:14b",
                size=8988124298,
                modified_at="2026-02-14T14:32:47.74589142Z",
            ),
        ]
    ...
    async def mock_chat(model, messages, stream=False, **kwargs):
        return {
            "message": {"role": "assistant", "content": "This is a test response from " + model}
        }
...
def mock_router_engine():
    ...
    async def mock_select_model(prompt, available_models):
        ...
        if "code" in prompt.lower() or "python" in prompt.lower():
            ...
            return RoutingResult(
                selected_model="llama3.2:1b", confidence=0.85, reasoning="General query"
            )
...
def configured_client(mock_backend, mock_router_engine):
    ...
    def get_settings_override():
        return Settings(admin_api_key=None, cache_enabled=True, signature_enabled=True)
...
class TestChatCompletions:
    ...
    def test_coding_prompt_routing(self, configured_client):
        ...
        response = configured_client.post(
            "/v1/chat/completions",
            json={"messages": [{"role": "user", "content": "Write Python code for binary search"}]},
        )
    ...
    def test_streaming_chat(self, configured_client):
        ...
        assert "data:" in content or response.headers.get("content-type", "").startswith(
            "text/event-stream"
        )
    ...
    def test_invalid_content_type(self, configured_client):
        ...
        response = configured_client.post(
            "/v1/chat/completions", data="not json", headers={"Content-Type": "text/plain"}
        )
    ...
    def test_empty_messages(self, configured_client):
        ...
        response = configured_client.post("/v1/chat/completions", json={"messages": []})
...
class TestErrorHandling:
    ...
    def test_model_generation_failure(self, configured_client, mock_backend):
        ...
        async def failing_chat(model, messages, stream=False, **kwargs):
            return {"message": {"role": "assistant", "content": "Response from " + model}}
...
class TestCaching:
    ...
    def test_routing_cache_hit(self, configured_client, mock_router_engine):
        ...
        cached_result = RoutingResult(
            selected_model="llama3.2:1b", confidence=0.9, reasoning="Cached decision"
        )
    ...
    def test_response_cache_hit(self, configured_client, mock_router_engine):
        ...
        mock_router_engine.semantic_cache.get_response = AsyncMock(return_value="Cached response")
...
class TestRequestValidation:
    ...
    def test_invalid_role(self, configured_client):
        ...
        response = configured_client.post(
            "/v1/chat/completions", json={"messages": [{"role": "invalid", "content": "Test"}]}
        )
    ...
    def test_xss_in_prompt(self, configured_client):
        ...
        response = configured_client.post(
            "/v1/chat/completions", json={"messages": [{"role": "user", "content": xss_prompt}]}
        )
